...
A sketch should have at least the two basic methods: `+setup+` and
`+draw+`. The `+setup+` method will be called once when the sketch first
starts. Then, the draw method will be called 60 times a second.
...
Note that the `+x+` and `+y+` variables are no longer inside the
`+draw+` method. They are `+global+` variables, which allows them to
retain their value after the method has finished. Change the code to
